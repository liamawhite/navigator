...
func NewProxyService(connectionManager ConnectionManager, logger *slog.Logger) *ProxyService {
	return &ProxyService{
		connectionManager: connectionManager,
		logger:            logger,
...
// GetProxyConfigByPod is deprecated - use GetProxyConfig with explicit cluster specification
func (p *ProxyService) GetProxyConfigByPod(ctx context.Context, namespace, podName string) (*types.ProxyConfig, error) {
	return nil, fmt.Errorf("automatic cluster discovery is deprecated - please specify cluster_id explicitly using GetProxyConfig")
}
...
// findClusterForPod searches all connected clusters to find which one contains the specified pod
func (p *ProxyService) findClusterForPod(namespace, podName string) (string, error) {
	allClusters := p.connectionManager.GetAllClusterStates()

	var foundClusters []string

	for clusterID, clusterState := range allClusters {
		if clusterState == nil {
			continue
		}

		// Search through all services in this cluster for instances matching the pod
		for _, service := range clusterState.Services {
			...
			for _, instance := range service.Instances {
				if instance.PodName == podName {
					foundClusters = append(foundClusters, clusterID)
					break // Found in this cluster, no need to check more instances in this service
				}
			}
		}
	}
	...
	if len(foundClusters) == 0 {
		return "", fmt.Errorf("pod %s/%s not found in any connected cluster", namespace, podName)
	}

	if len(foundClusters) > 1 {
		p.logger.Warn("pod found in multiple clusters, using first one",
			"namespace", namespace,
			"pod", podName,
