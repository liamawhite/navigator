...
func (m *mockConnectionManager) UpdateCapabilities(clusterID string, capabilities *v1alpha1.EdgeCapabilities) error {
	if !m.connections[clusterID] {
		return status.Errorf(codes.NotFound, "connection not found")
	}
...
func TestManagerServer_processClusterIdentification(t *testing.T) {
	logger := logging.For("test")
	config := &mockConfig{port: 8080, maxMessageSize: 10485760}
	connectionManager := newMockConnectionManager()

	server, err := NewManagerServer(config, connectionManager, logger)
	if err != nil {
	...
	tests := []struct {
		...
		{
			name: "valid cluster identification with capabilities",
			req: &v1alpha1.ConnectRequest{
				Message: &v1alpha1.ConnectRequest_ClusterIdentification{
					ClusterIdentification: &v1alpha1.ClusterIdentification{
						ClusterId: "test-cluster-with-metrics",
			...
			expectedID:  "test-cluster-with-metrics",
			expectError: false,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			clusterID, capabilities, err := server.processClusterIdentification(tt.req)
			...
			// Suppress unused variable warning for capabilities
			_ = capabilities
		})
	}
}

func TestManagerServer_processClusterStateUpdate(t *testing.T) {
	logger := logging.For("test")
	config := &mockConfig{port: 8080, maxMessageSize: 10485760}
	connectionManager := newMockConnectionManager()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := server.processClusterStateUpdate(tt.clusterID, tt.req)
...
func TestManagerServer_StartStop(t *testing.T) {
	logger := logging.For("test")
	...
	err = server.Start()
	if err != nil {
		t.Errorf("Expected no error starting server, got: %v", err)
	}
	...
	err = server.Stop()
	if err != nil {
		t.Errorf("Expected no error stopping server twice, got: %v", err)
	}
}
